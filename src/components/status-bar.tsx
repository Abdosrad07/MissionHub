  ...
  return (
    <header
      className={`fixed inset-x-0 top-0 z-50 border-b transition-all duration-300 ${
        scrolled
          ? "border-border bg-noir/95 backdrop-blur-md"
    ...
    >
      <div className="mx-auto flex h-12 max-w-6xl items-center justify-between gap-4 px-4 sm:px-6">
        ...
        <div className="flex items-center gap-3 font-mono text-[11px] tracking-wider text-muted">
          ...
          <a
            ...
            className="hidden items-center gap-1.5 border border-border px-2 py-1 text-ink transition-colors hover:border-signal hover:text-signal sm:flex"
          >
          ...
          <button
            ...
            className="flex h-8 w-8 flex-col items-center justify-center gap-1 border border-border text-ink transition-colors hover:border-line2 md:hidden"
          >
            <span
      ...
      <div
        id="mobile-nav"
        className={`overflow-hidden border-t border-border bg-noir/95 backdrop-blur-md transition-[max-height] duration-200 md:hidden ${
        ...
        <nav className="flex flex-col px-4 py-3 sm:px-6">
          {links.map((l) => (
            <Link
              ...
              href={l.href}
              onClick={() => setOpen(false)}
              className="border-b border-border/60 py-3 font-mono text-xs tracking-widest text-muted transition-colors last:border-0 hover:text-signal"
            >
              {l.label}
            </Link>
          ))}
