...
export function ArchitectureDiagram({
  ...
  return (
    <div className="grid gap-4 lg:grid-cols-[1fr_0.9fr]">
      {/* ── Diagramme / liste ──────────────────────────── */}
      <div className="relative min-h-[340px] rounded-lg border border-line bg-panel2/60 p-4 sm:p-6">
            ...
            {paths.map((d, i) => {
              ...
              return (
                <path
                  ...
                  d={d}
                  fill="none"
                  stroke={active ? accent : "var(--color-line)"}
            ...
            {layers.map((l, i) => (
              ...
              >
                <span className="flex items-center gap-3">
                  <span
                    className="flex h-6 w-6 flex-none items-center justify-center rounded border border-line bg-panel2 font-mono text-[10px]"
          ...
          {layers.map((l, i) => (
            <button
              ...
              className={`rounded-md border px-3.5 py-2.5 text-left transition-colors ${
                selected === i ? "bg-noir" : "bg-panel"
              }`}
              style={selected === i ? { borderColor: accent } : undefined}
            >
              <span className="flex items-center gap-2.5">
                <span className="flex h-5 w-5 flex-none items-center justify-center rounded border border-line bg-panel2 font-mono text-[9px] text-muted">
                  L{i}
                </span>
                <span
      ...
      {/* ── Détail de la couche ────────────────────────── */}
      <aside
        className="flex flex-col rounded-lg border border-line bg-panel p-5 sm:p-6"
        aria-live="polite"
      >
              ...
              {selectedConns.map(({ l, i }) => (
                <li key={i}>
                  <button
                    type="button"
                    onClick={() => setSelected(i)}
                    className="flex w-full items-center gap-2 rounded border border-line bg-panel2 px-2.5 py-1.5 text-left font-mono text-[11px] text-muted transition-colors hover:border-line2 hover:text-ink"
                  >
