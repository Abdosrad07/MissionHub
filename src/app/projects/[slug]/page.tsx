    ...
    <main>
      ...
      <section className="mx-auto max-w-6xl px-4 pt-28 sm:px-6 sm:pt-32">
        ...
        >
          ...
          <Link href="/#projets" className="transition-colors hover:text-signal">
            PROJETS
          </Link>
          <span aria-hidden>/</span>
        ...
        <div className="mt-6 grid items-start gap-10 lg:grid-cols-[1fr_0.85fr]">
          <div>
            <div className="flex flex-wrap items-center gap-3 font-mono text-[11px] tracking-widest">
              <span className="border border-line px-2 py-1 text-muted">{project.channel}</span>
              <span
            ...
            <p className="mt-3 font-mono text-xs tracking-widest" style={{ color: project.accent }}>
              {project.category}
            </p>
            ...
            <dl className="mt-8 grid grid-cols-1 gap-px overflow-hidden border border-line bg-line sm:grid-cols-2">
              <div className="bg-panel p-4">
                <dt className="font-mono text-[10px] uppercase tracking-widest text-muted">Contexte</dt>
                <dd className="mt-1.5 text-sm text-ink">{project.context}</dd>
              </div>
              <div className="bg-panel p-4">
                <dt className="font-mono text-[10px] uppercase tracking-widest text-muted">Mon rôle</dt>
            ...
            {/* Boutons principaux — visibles, alignés, système */}
          ...
          {/* Média vedette : réel, sinon monitoring */}
          <div>
            {featured ? (
              <figure className="media-frame scanline">
                ...
                <figcaption className="flex items-center justify-between gap-3 font-mono text-[11px] tracking-wide text-muted">
                  <span className="flex items-center gap-2">
                    {featured.kind === "video" ? (
                      <PlayIcon className="h-3.5 w-3.5" style={{ color: project.accent }} />
                    ) : (
                      <CameraIcon className="h-3.5 w-3.5" style={{ color: project.accent }} />
                    )}
                    {featured.caption}
                  </span>
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    {project.metrics.map((m) => (
                      <div key={m.label} className="border border-line bg-panel2 p-3">
                        <p className="font-mono text-xl font-semibold" style={{ color: project.accent }}>
                          {m.value}
                        </p>
      ...
      <section className="mx-auto max-w-6xl px-4 pt-20 sm:px-6">
        <Reveal>
          <SectionHeading cmd={`grep -n "probleme" ${project.slug}.log`} title="Le problème" />
        </Reveal>
        <Reveal>
      ...
      <section className="mx-auto max-w-6xl px-4 pt-20 sm:px-6">
        <Reveal>
          <SectionHeading cmd={`./${project.slug} --mode solution`} title="La solution" />
        </Reveal>
        <div className="grid gap-4 md:grid-cols-3">
          {project.solution.map((s, i) => (
            <Reveal key={s} delay={i * 70}>
              <div className="panel h-full p-5">
                <p className="font-mono text-[11px] tracking-widest" style={{ color: project.accent }}>
                  S{i + 1}
                </p>
      ...
      {/* ══ 6. ARCHITECTURE INTERACTIVE ═══════════════════ */}
      <section className="mx-auto max-w-6xl px-4 pt-20 sm:px-6">
        <Reveal>
          <SectionHeading cmd={`trace --layers ${project.slug}`} title="Architecture" />
        </Reveal>
        <Reveal>
          <p className="mb-5 max-w-2xl font-mono text-[11px] leading-relaxed tracking-wide text-muted">
            {"// Interactif : sélectionnez une couche pour l'explorer et voir ses connexions — le flux réseau passe par les lignes surlignées."}
          </p>
          <ArchitectureDiagram layers={project.architecture} accent={project.accent} />
        </Reveal>
      </section>
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {project.stack.map((g, i) => (
            <Reveal key={g.label} delay={(i % 3) * 60}>
              <div className="panel h-full p-5">
                <p className="font-mono text-[10px] uppercase tracking-widest text-muted">{g.label}</p>
                <div className="mt-3 flex flex-wrap gap-1.5">
                  {g.items.map((t) => (
                    <span
                      key={t}
                      className="border border-line bg-panel2 px-2 py-0.5 font-mono text-[11px] tracking-wider text-ink"
                    >
                      {t}
                    </span>
                  ))}
      ...
      {/* ══ 8. CAPTURES D'ÉCRAN & MÉDIAS ══════════════════ */}
      <section className="mx-auto max-w-6xl px-4 pt-20 sm:px-6">
        <Reveal>
          <SectionHeading cmd="open ./captures" title="Captures & médias" />
        </Reveal>
        {project.media.length > 0 ? (
              ...
              <Reveal key={m.src} delay={(i % 2) * 60}>
                <figure className="media-frame h-full">
                  ...
                  <figcaption className="flex items-center gap-2 font-mono text-[11px] tracking-wide text-muted">
                    {m.kind === "video" ? (
                      <PlayIcon className="h-3.5 w-3.5 flex-none" style={{ color: project.accent }} />
                    ) : (
                      <CameraIcon className="h-3.5 w-3.5 flex-none" style={{ color: project.accent }} />
                    )}
                    {m.caption}
                  </figcaption>
                </figure>
          ...
          <div className="panel flex flex-col items-center gap-2 px-6 py-10 text-center">
            <p className="font-mono text-[11px] tracking-widest text-warn">[ NO-CAPTURE ]</p>
            <p className="max-w-xl text-sm leading-relaxed text-muted">{project.mediaNote}</p>
          </div>
        )}
        {project.media.length > 0 && (
          <p className="mt-4 font-mono text-[11px] tracking-wide text-muted">
            {"// Toutes les captures et vidéos sont servies directement depuis le dépôt GitHub — rien n'est recréé ni retouché."}
          </p>
        )}
      </section>
      ...
      {/* ══ 10. VERDICT — CE QUE LE DÉPÔT MONTRE ══════════ */}
      <section className="mx-auto max-w-6xl px-4 pt-20 sm:px-6">
        <Reveal>
          <SectionHeading cmd="audit --honest" title="Ce que le dépôt montre — et ne montre pas" />
        </Reveal>
            ...
            <div className="panel h-full p-6">
              ...
              <ul className="mt-4 space-y-3">
                {project.verdict.repoShows.map((v) => (
                  <li key={v} className="flex gap-2.5 text-sm leading-relaxed text-muted">
      ...
      <section className="mx-auto max-w-6xl px-4 pt-20 sm:px-6">
        <Reveal>
          <SectionHeading cmd="netstat -tlnp | grep abdosrad" title="Liens" />
        </Reveal>
        <Reveal>
          <div className="panel divide-y divide-line">
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="group flex items-center justify-between gap-4 px-6 py-4 transition-colors hover:bg-panel2"
            >
              <div className="flex items-center gap-3">
                <GitHubIcon className="h-5 w-5 flex-none text-signal" />
                <div>
                  <p className="text-sm font-semibold text-ink">Dépôt GitHub — {project.title}</p>
                  <p className="font-mono text-[11px] text-muted">{project.githubUrl}</p>
                </div>
              </div>
            ...
            {project.docsLinks?.map((d) => (
              ...
              >
                <div>
                  ...
                  <p className="truncate font-mono text-[11px] text-muted">{d.url}</p>
                </div>
      ...
      <nav className="mx-auto mt-24 max-w-6xl px-4 sm:px-6" aria-label="Autres projets">
        <div className="grid gap-4 sm:grid-cols-2">
          {[
            ...
            >
              ...
              <p className="mt-1 text-lg font-semibold text-ink transition-colors group-hover:text-signal">
                {label === "PRÉCÉDENT" ? "← " : ""}
                {p.title}
                {label === "SUIVANT" ? " →" : ""}
              </p>
